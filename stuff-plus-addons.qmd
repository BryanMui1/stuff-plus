...
data_original <- read_csv("/Users/willwhitis/bryan/stuffdata.csv")
...
hybrid_quality <- function(PitchCall, PlayResult, ExitSpeed = NA, Angle = NA, Direction = NA) {
  ...
  library(dplyr)
  callvalues <- main %>%
  mutate(Top_Bottom = as.factor(Top_Bottom)) %>%
  arrange(GameID, Inning, Top_Bottom, PAofInning) %>%
  group_by(GameID, Inning, Top_Bottom) %>%
  mutate(runs_this_inning = cumsum(RunsScored)) %>%
  ungroup() %>%
  select(PitchCall, PlayResult, GameID, Inning, Top_Bottom, PAofInning, RunsScored, runs_this_inning) %>%
  arrange(GameID, Inning, Top_Bottom, PAofInning)
 
  # Step 1: Build the PitchCall values
pitch_vals <- callvalues %>%
  filter(!is.na(PitchCall)) %>%
  group_by(PitchCall) %>%
  summarise(run_value = mean(runs_this_inning, na.rm = TRUE), .groups = "drop") %>%
  rename(Outcome = PitchCall)

# Step 2: Run value by PlayResult
play_vals <- callvalues %>%
  filter(!is.na(PlayResult)) %>%
  group_by(PlayResult) %>%
  summarise(run_value = mean(runs_this_inning, na.rm = TRUE), .groups = "drop") %>%
  rename(Outcome = PlayResult)

# Step 3: Combine both
all_run_values <- bind_rows(pitch_vals, play_vals) %>%
 ...
 base_score <- case_when(
  ...
  has_contact_data <- !is.na(main$ExitSpeed) & !is.na(main$Angle)
 
  #Create barrel model to rate contact
  main <- main %>%
  mutate(
    is_xb_hit = PlayResult %in% c("Double", "Triple", "HomeRun")) 
 
  barrel_model <- glm(
  is_xb_hit ~ ExitSpeed + Angle + I(Angle^2),
  data = main[has_contact_data,],
  family = binomial(link = "logit")
)

# Add predicted xBA to data
  main <- main[has_contact_data,] %>%
  mutate(xba = predict(barrel_model, type = "response"))
...
main <- main %>%
  ...
  ungroup() %>%
  mutate(
    csw_league = if_else(pitch_index < 1000, mean(main$is_csw[1:1000]), csw_league)         # apply 20-pitch min for league CSW
  ) %>%
